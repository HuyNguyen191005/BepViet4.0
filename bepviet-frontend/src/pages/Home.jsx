...
const Home = () => {
    ...
    useEffect(() => {
        ...
        // 2. Gọi API lấy danh sách món ăn
        axiosClient.get('/recipes')
            .then(res => {
                // Backend Laravel đã tự động trả về cột 'slug' trong response
                setRecipes(res.data);
            })
            .catch(err => {
                console.error("Lỗi tải món ăn:", err);
                // Dữ liệu mẫu fallback (Đã thêm slug giả lập)
                setRecipes([
                    { 
                        recipe_id: 1, 
                        title: 'Phở Bò Tái Lăn', 
                        slug: 'pho-bo-tai-lan', // <--- THÊM DÒNG NÀY
                        cooking_time: 45, 
                        difficulty: 'Dễ', 
                        image_url: '', 
                        author: { full_name: 'Bếp Trưởng' } 
                    },
                    { 
                        recipe_id: 2, 
                        title: 'Sườn Xào Chua Ngọt', 
                        slug: 'suon-xao-chua-ngot', // <--- THÊM DÒNG NÀY
                        cooking_time: 30, 
                        difficulty: 'Trung bình', 
                        image_url: '', 
                        author: { full_name: 'Mẹ Bắp' } 
                    },
                ]);
            });
    }, []);
